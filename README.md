...
```json
{"addon":"8d8b8ba4_dropbox_sync","input":{"command":"upload"}}
```
...
*Note*: The hash `8d8b8ba4` that is prepended to the `dropbox_sync` add-on slug above is required. [See below](#repository-slugs-in-hassio) for an explanation.
### Configuration
...
key = "https://github.com/dancwilliams/hassio-addons"
